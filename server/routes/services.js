...
router.post('/', authRequired, requireRole('admin'), requirePermission('services'), (req, res) => {
  const { title, summary, body, icon, cover_image, show_cover, code, language = 'tr', status = 'draft', sort_order = 0, slug } = req.body || {};
  if (!isNonEmptyString(title, 200)) return res.status(400).json({ error: 'invalid_input' });
  const finalSlug = (slug && slugify(slug)) || slugify(title);
  try {
    const info = db.prepare(`
      INSERT INTO services (slug, code, title, summary, body, icon, cover_image, show_cover, language, sort_order, status, author_id)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(finalSlug, code || null, title, summary || null, body || '', icon || null, cover_image || null,
      show_cover === undefined ? 1 : (show_cover ? 1 : 0),
...
router.put('/:id', authRequired, requireRole('admin'), requirePermission('services'), (req, res) => {
  ...
  const { title, summary, body, icon, cover_image, show_cover, code, language, status, sort_order, slug } = req.body || {};
  const next = {
  ...
  try {
    db.prepare(`UPDATE services SET
      slug=?, code=?, title=?, summary=?, body=?, icon=?, cover_image=?, show_cover=?, language=?, status=?, sort_order=?, updated_at=CURRENT_TIMESTAMP
      WHERE id=?`).run(next.slug, next.code, next.title, next.summary, next.body, next.icon, next.cover_image,
      next.show_cover, next.language, next.status, next.sort_order, req.params.id);
    res.json({ ok: true });
  } catch (e) {
    if (String(e.message).includes('UNIQUE')) return res.status(409).json({ error: 'slug_exists' });
    throw e;
